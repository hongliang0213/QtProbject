...
void InputConvertNormal::mouseEvent(const QMouseEvent *from, const QSize &frameSize, const QSize &showSize)
{
    ...
    AndroidMotioneventAction action;  //判断鼠标事件具体是按下还是什么,进行对应的转化
    switch (from->type()) {
    ...
    // set data
    ControlEvent * controlEvent = new ControlEvent(ControlEvent::CET_MOUSE);  //new 出事件类型，进行对应数据封装

    //  frameSize  实际视屏帧的大小
    controlEvent->setMouseEventData(action,covertMouseButtons(from->buttons()),QRect(pos.toPoint(),frameSize));  //
    sendControlEvent(controlEvent);   // 触发事件进行发送
...
void InputConvertNormal::keyCodeEvent(const QKeyEvent *from, const QSize &frameSize, const QSize &showSize)
{
    if (!from) {
           return;
       }
...
//将qt鼠标按键转换为控制指令的鼠标按键
AndroidMotioneventButtons InputConvertNormal::covertMouseButtons(Qt::MouseButtons buttonStatus)
{
    quint32 buttons =0;
    if(buttonStatus & Qt::LeftButton){
        buttons |= AMOTION_EVENT_BUTTON_PRIMARY;  //左
    }
    if(buttonStatus & Qt::RightButton){
        buttons |= AMOTION_EVENT_BUTTON_SECONDARY; //右
    }
    if(buttonStatus & Qt::MiddleButton){
        buttons |= AMOTION_EVENT_BUTTON_TERTIARY; //中
    }
    if(buttonStatus & Qt::XButton1){
        buttons |= AMOTION_EVENT_BUTTON_BACK;   //扩展
    }
    if(buttonStatus & Qt::XButton2){
        buttons |= AMOTION_EVENT_BUTTON_FORWARD;
    }
    return (AndroidMotioneventButtons)buttons;
}

//  键盘key  code 判断按下的是什么键，进行对应的转换
